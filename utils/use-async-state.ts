...
const defaultRejectHandler = (event: AsyncDispatchRejectEvent<any>): void => {
  ...
  throw reason instanceof Error
    ? reason
    : new Error(`Uncaught rejection: ${reason}`);
};
...
const useAsyncState = <T>(
  ...
  T,
  AsyncDispatch<T | Promise<T> | ((prevState: T) => T | Promise<T>)>,
  boolean
] => {
  ...
  >((next) => {
    ...
    const { resetBeforeTask = true, onReject = defaultRejectHandler } =
      optionRef.current;

    const value =
      typeof next === "function"
        ? (next as (prevState: T) => T | Promise<T>)(
